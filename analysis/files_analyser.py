...
import pandas as pd
from analysis.LLM_analyzer import ChatGPTApi
...
def get_important_extensions(list_files : List[Path] , extensions = []) -> List[str]:
    ...
    if not extensions:
        ...
        while df[df['extension'] == most_common_suffix_list[idx]].shape[0] <= 0 and idx < len(most_common_suffix_list):
            idx +=1
